...
ngAfterViewInit() {
        if (this.flexGrid && this.flexGrid.columns && this.flexGrid.columns.length) {
          this.changedColumnWidth(this.flexGrid.columns, 'Id');
            this.flexGrid.rows.forEach((worksheetRow) => {
                    if (worksheetRow.dataItem && worksheetRow.hasChildren) {
                            this.changeRowColor(worksheetRow);
                    }
                });
        }
}
changeRowColor(worksheetRow: GroupRow){
worksheetRow.isCollapsed = !worksheetRow.isCollapsed;
}
